...
// Helpful runtime warning for missing but commonly-needed env vars
if (!env.DATABASE_URL) {
  // do not throw — allow dev to run with sqlite or to set later
  console.warn('[env] DATABASE_URL is not set. DB operations will fail until configured.')
}
